...
test('shared tab pages match Today with an open leading top bar', async () => {
  ...
  assert.match(source, /nativeHeaderGlassButtonModifiers/);
  assert.match(source, /frame\(\{ width: 44, height: 44 \}\)/);
  assert.match(source, /contentShape\(shapes\.circle\(\)\)/);
  assert.match(source, /glassEffect\(\{ glass: \{ variant: 'regular', interactive: true \}, shape: 'circle' \}\)/);
  assert.match(today, /\.\.\.nativeHeaderGlassButtonModifiers\('Workout actions'\)/);
  assert.match(today, /<NativeAppScreenTopbar/);
  ...
  const todayScreen = await readFile(new URL('../app/(tabs)/today/index.tsx', import.meta.url), 'utf8');
  const todayLayout = await readFile(new URL('../app/(tabs)/today/_layout.tsx', import.meta.url), 'utf8');
  const exerciseScreen = await readFile(new URL('../app/(tabs)/today/exercise.tsx', import.meta.url), 'utf8');
  ...
  assert.match(todayLayout, /<Stack/);
  assert.match(todayLayout, /<Stack\.Screen name="exercise"/);
  assert.match(todayScreen, /pathname: '\/today\/exercise'/);
  assert.match(exerciseScreen, /<NativeExercisePage/);
  assert.match(today, /fitToContents=\{activeSheet\?\.type === 'metric'\}/);
  assert.match(today, /activeSheet\?\.type === 'metric' \? \[\] : \[presentationDetents\(\[sheetDetent\]\)\]/);
  assert.match(today, /pickerStyle\('wheel'\), frame\(\{ maxWidth: 1000, height: 216 \}\)/);
  ...
  assert.doesNotMatch(today, /sheetExerciseIndex|visibleExerciseIndex|sheetPage/);
