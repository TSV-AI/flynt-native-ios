...
Current TestFlight verification: App Store Connect accepted build `1.0.0 (3)` on 2026-08-06 and attached it to the internal Team group. Production build `1.0.0 (4)` completed from commit `9d23483` as EAS build `71fc8450-a15e-4d0b-af09-6c38f71c8d77`; EAS submission `1d7df58c-4563-4470-aaf0-85ed378748e7` successfully uploaded the binary to App Store Connect, where Apple processing is pending. The owner tester is invited. Installation and an owner journey on build 4 remain unverified.
...
- Verified in EAS Build on 2026-08-06: production Store build `1.0.0 (3)`
  ...
  key were created. Submission `fc433bb7-8d98-49c3-b4f9-be59d8799939`
  completed, and App Store Connect accepted build `1.0.0 (3)`. Apple extracted
  and displayed its embedded App Store icon. The build is attached to the
  internal Team group, the owner Apple ID is invited, and focused beta test
  instructions are saved. TestFlight installation is not yet verified. The
  initially generated App
  ...
  save-verified as `Personalized strength training`. The live policy at
  `https://flynt.training/privacy` was verified in the browser and entered as
  the TestFlight privacy URL. The external `External Beta` group exists, and
  build 3 reached its Beta App Review information form. External submission is
  blocked on the review contact phone and reviewer sign-in credentials; no
...
- [~] Publish an accessible privacy policy URL and keep the same policy easy to reach inside the app.
  Evidence: `https://flynt.training/privacy` was live-verified on 2026-08-06 and saved as the TestFlight privacy URL. It includes collection, use, service-provider, storage and security, user-control, and policy-change disclosures. A final binary check of the in-app link remains required.
