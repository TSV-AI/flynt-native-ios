...
Current TestFlight verification: Production build `1.0.0 (4)` completed from commit `9d23483` as EAS build `71fc8450-a15e-4d0b-af09-6c38f71c8d77`; EAS submission `1d7df58c-4563-4470-aaf0-85ed378748e7` successfully uploaded the binary to App Store Connect. Apple processing completed, the binary state is Validated, non-exempt encryption is No, and build 4 is attached to the internal `Team (Expo)` group with one tester. Build `1.0.0 (3)` remains available as the rollback build. Installation and an owner journey on build 4 remain unverified.
